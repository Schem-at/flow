...
const PRIORITY = ['Schematic', 'Schemati', 'Field', 'Image', 'Random', 'Noise', 'Vec3', 'Vec2', 'SchematicUtils', 'Progress', 'Easing', 'Calculator', 'Pathfinding', 'Logger', 'Image', 'SchematicBuilder', 'Diff', 'Fingerprint'];

export function getApiDocs(): ApiGroup[] {
  if (cache) return cache;
  const runtime = parseAmbientDts(AMBIENT_DTS, 'runtime');
  // Since nucleation 0.3.0 the nucleation ambient dts is hand-authored around
  // the endowed compat `Schematic` class (nucleationAmbient.ts) — it parses
  // directly, no wrapper-grafting needed.
